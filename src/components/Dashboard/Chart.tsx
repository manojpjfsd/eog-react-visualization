...
const colors = ['#581845', '#C70039', '#FF5733', '#FFC300', '#900C3F', '#DAF7A6'];

const calcThirtyMinutesAgo: any = (): any => {
  const value: any = new Date();
  return value - 30 * 60 * 1000;
};
const thirtyMinutesAgo = calcThirtyMinutesAgo();

export default _props => {
  const dispatch = useDispatch();
  const metrics: any = useSelector(Packs.metrics.selectors.getMetrics);
  ...
  const trafficSeries = useSelector(Packs.measurements.selectors.getTrafficSeries);
  ...
  return (
    <Resizable>
      <ChartContainer
        timeRange={trafficSeries.timerange([calcThirtyMinutesAgo(), new Date().getTime()])}
            ...
            {series.map((metricSeries, i) => {
              ...
              return (
                <LineChart
                  key={i}
                  style={style}
                  axis={metricSeries && metricSeries.atLast().get('unit')}
                  series={metricSeries}
                  column={['value']}
                />
              );
